#' @title Counting/removing of NA's per column or row vector
...
#' @examples x <- data.frame(NR = c(1,2,3,5,NA,6,7,NA),Letters = c(rep(LETTERS)[1:7],NA))
...
na_action <- function(df, margin = 2, drop = F, cutoff = 1) {
  ...
  if(is.matrix(df) | is.data.frame(df))
  {
    if(margin == 2){
      ...
      class(x) <- c("CountClassCol", class(x))
      return(x)
    } 
    if(margin == 1){
        x <- apply(df, 1, function(z) sum(is.na(z)))
        class(x) <- c("CountClassRow", class(x))
...
print.CountClassCol <- function(obj) {
  cat("Column count NAs\n-----------------\n")
  attributes(obj)$class <- NULL
  NextMethod(obj)
}

print.CountClassRow <- function(obj) {
  cat("Row count NAs\n-----------------\n")
  attributes(obj)$class <- NULL
  NextMethod(obj)
}
